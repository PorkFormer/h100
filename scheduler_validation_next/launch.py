...
p=argparse.ArgumentParser();p.add_argument('--case',required=True);p.add_argument('--scheduler',default='fixed_wave');p.add_argument('--concurrency',type=int,default=4);p.add_argument('--kind',choices=['perf','collect'],default='perf');p.add_argument('--entry',default='dapo');p.add_argument('--mode',default='replace');p.add_argument('--loss',default='vanilla');p.add_argument('--count',type=int,default=256);p.add_argument('--natural',action='store_true');a=p.parse_args()
...
assert uuids==json.loads((R/'preflight.json').read_text())['uuid_order']
...
if a.kind=='collect':
    cmd=[sys.executable,str(R/'collect.py'),'--case',a.case]
elif a.kind=='nccl':
    ...
    cmd=[sys.executable,str(R/'perf_case.py'),'--case',a.case,'--scheduler',a.scheduler,'--concurrency',str(a.concurrency),'--count',str(a.count)]+(['--natural'] if a.natural else [])
# Preserve exact executed code separately from later runner fixes.
import hashlib
snapshots={}
for path in [R.parent/'scheduler_validation/process_cleanup.py',R/'perf_case.py',R/'collect.py',R/'workload.py',R/'launch.py',
...
with (E/f'{a.case}.log').open('x') as log:
    ...
    while proc.poll() is None:
        ...
        used=[int(v.split(',')[0]) for v in values.splitlines()];util=[int(v.split(',')[1]) for v in values.splitlines()];peak=[max(x,y) for x,y in zip(peak,used,strict=True)]
        samples.append(dict(seconds=time.monotonic()-start,epoch=time.time(),mib=used,utilization=util))
        if time.monotonic()-start>(14400 if a.kind=='collect' else 3600):
...
sys.path.insert(0,str(R.parent/'scheduler_validation'))
