...
p=argparse.ArgumentParser();p.add_argument('--h',type=int,default=128);p.add_argument('--l',type=int,default=512);p.add_argument('--count',type=int,default=8);a=p.parse_args()
root=Path(__file__).resolve().parent;out=root/'evidence';case=f'h{a.h}'
...
for i,uuid in enumerate(uuids):
    ...
    f=(out/f'{case}_replica{i}.log').open('x');cmd=[sys.executable,str(root/'infer.py'),'--replica',str(i),'--h',str(a.h),'--l',str(a.l),'--count',str(a.count)]
