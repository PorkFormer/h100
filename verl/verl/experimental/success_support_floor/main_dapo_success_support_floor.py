"""Dedicated entry point for synchronous DAPO with BSSF."""
...
class SuccessSupportFloorTaskRunner(TaskRunner):
    """Build current-API workers and select only the BSSF trainer."""

    def run(self, config):
        pprint(OmegaConf.to_container(config, resolve=True))
        print(f"SuccessSupportFloorTaskRunner hostname: {socket.gethostname()}, PID: {os.getpid()}")
        ...
        trainer = RayDAPOSuccessSupportFloorTrainer(
            config=config,
            tokenizer=tokenizer,
            processor=processor,
...
def main(config):
    ...
    runner_class = ray.remote(num_cpus=1)(SuccessSupportFloorTaskRunner)
