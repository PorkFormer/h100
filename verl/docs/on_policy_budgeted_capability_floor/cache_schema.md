...
`build_floor_cache.py` requires `--event-equivalence-attestation`. The schema-1 attestation must have `passed=true`, the same budget and tokenizer/template/verifier fingerprints, exact prompt/rollout/historical-score file-set hashes, exact prompt/rollout/historical/recomputed row counts, zero directional mismatches, zero historical and recomputed errors, and `exact_match_count == row_count` for every rollout and historical-score row. Its protocol fingerprint is copied into the cache manifest only after independently recomputing and matching it. Missing or failed attestations and all source, row-count, budget, verifier, or protocol mismatches fail closed.
...
For event-equivalence recomputation, the validator also normalizes the frozen 512-prompt legacy layout in memory: `canonical_prompt` supplies `raw_prompt`, `extra_info_json` supplies `extra_info`, and a domain-separated SHA-256 digest of exact response token IDs supplies a missing `response_hash`; an existing response hash must equal that digest. Historical rows inherit only identity fields that exactly agree with their bound rollout row. Original artifacts are never rewritten, and their original file-set hashes remain the attested source hashes. Reward-pipeline `error` or `timeout` metadata is preserved across batch rows and is a recomputation error even when the manager also emits `acc=0`.
