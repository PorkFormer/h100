...
def test_k_sample_aggregation():
    ...
    diagnostics = aggregate_probe_diagnostics(
        ...
        probe_rewards=[1.0, 0.0],
        original_rewards=[0.0],
...
def test_logging_statistics_false_negative_and_token_overhead():
    ...
    diagnostics = aggregate_probe_diagnostics(
        ...
        generations=generations,
        probe_rewards=[1.0, 0.0, 1.0, 1.0],
        original_rewards=[1.0, 0.0, 1.0],
    ...
    assert metrics["probe/reward_mean"] == pytest.approx(0.75)
    assert metrics["probe/truncation_false_negative_candidate_rate"] == pytest.approx(0.5)
    assert metrics["probe/truncation_high_confidence_recoverable_rate"] == pytest.approx(0.5)
