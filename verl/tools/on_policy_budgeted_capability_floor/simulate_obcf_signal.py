...
def simulate_obcf_signal(
    *,
    ...
    deficits_by_count: dict[int, list[float]] = defaultdict(list)
    for group_id, prompt_id in enumerate(protected_ids):
        base_count = int(protected_by_id[prompt_id]["base_prefix_success_count"])
        deficits_by_count[base_count].append(float(result.deficit[group_id].item()))
    deficit_report = {
        str(count): {
            "prompt_count": len(values),
            "deficit_mean": sum(values) / len(values),
        }
        for count, values in sorted(deficits_by_count.items())
    }
    ...
    report: dict[str, Any] = {
        ...
        "deficit_by_base_success_count": deficit_report,
        ...
        "engineering_gate": {
            ...
            "passed": nonzero_fraction >= ENGINEERING_GATE_THRESHOLD,
            "note": "Engineering density gate only; this is not a theoretical threshold.",
        },
    }
...
def main() -> None:
    ...
    if args.output:
        ...
        print(encoded, end="")
    if not report["engineering_gate"]["passed"]:
        raise SystemExit(2)
