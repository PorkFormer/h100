...
p=argparse.ArgumentParser();p.add_argument('--case',required=True);p.add_argument('--scheduler',choices=['fixed_wave','work_conserving'],required=True);p.add_argument('--concurrency',type=int,choices=[4,8],required=True);a=p.parse_args()
...
class Server(vLLMHttpServer):
    ...
    def receipt(self):
        return dict(pid=os.getpid(),cuda_visible=os.environ['CUDA_VISIBLE_DEVICES'])
...
async def main():
    ...
    frozen=torch.load(source/'replay_h2048_v2/actor_input.pt',weights_only=False)
    batch=frozen['batch']
    ...
    try:
        ...
        assert len(capture.requests)==78
        ...
        (OUT/'result.json').write_text(json.dumps(dict(status='PASS',scheduler=a.scheduler,concurrency=a.concurrency,
            startup_seconds=startup,warmup_seconds=warmup,continuation_seconds=seconds,
