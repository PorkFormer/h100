...
'data.train_files':'/tmp/ncbr_8gpu_scale_20260911/validation/evidence/prompts.parquet','data.val_files':'/tmp/ncbr_8gpu_scale_20260911/validation/evidence/prompts.parquet','data.max_prompt_length':1024,'data.max_response_length':2048,'data.train_batch_size':2 if a.entry=='dapo' else 64,'data.shuffle':False,'data.dataloader_num_workers':0,
'actor_rollout_ref.model.path':'/workspace/models/Qwen3-1.7B-Base','actor_rollout_ref.model.enable_gradient_checkpointing':True,
'actor_rollout_ref.actor.strategy':'fsdp','actor_rollout_ref.actor.ppo_mini_batch_size':2 if a.entry=='dapo' else 64,'actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu':1,'actor_rollout_ref.actor.ppo_epochs':1,'actor_rollout_ref.actor.optim.lr':1e-6,'actor_rollout_ref.actor.use_kl_loss':a.ref,'actor_rollout_ref.actor.kl_loss_coef':0.,'actor_rollout_ref.actor.policy_loss.loss_mode':a.loss,'actor_rollout_ref.actor.fsdp_config.use_torch_compile':False,
...
'actor_rollout_ref.rollout.boundary_return.mode':a.mode,'actor_rollout_ref.rollout.boundary_return.long_response_length':8192,'actor_rollout_ref.rollout.boundary_return.scheduler':a.scheduler,'actor_rollout_ref.rollout.boundary_return.max_concurrent_requests':a.concurrency,'actor_rollout_ref.rollout.boundary_return.request_batch_size':8,'actor_rollout_ref.rollout.boundary_return.request_timeout_seconds':600.,'actor_rollout_ref.rollout.boundary_return.long_reward_chunk_size':3,'actor_rollout_ref.rollout.boundary_return.seed':42,
...
'trainer.n_gpus_per_node':8,'trainer.nnodes':1,'trainer.total_training_steps':1 if a.ref else 4,'trainer.total_epochs':1,'trainer.val_before_train':False,'trainer.test_freq':-1,'trainer.save_freq':-1,'trainer.logger':['console'],'trainer.project_name':'ncbr_validation','trainer.experiment_name':case.name,'trainer.default_local_dir':str(case/'artifacts'),'trainer.resume_mode':'disable',
'ray_kwargs.ray_init.address':'local','ray_kwargs.ray_init._temp_dir':os.environ['RAY_TMPDIR'],'ray_kwargs.ray_init.namespace':case.name,'ray_kwargs.ray_init.num_cpus':48,'ray_kwargs.ray_init.include_dashboard':False,
}
if a.entry=='dapo':settings.update({'data.gen_batch_size':128,'algorithm.filter_groups.max_num_gen_batches':4})
